...
/**
 * Account is a bank account with basic services for deposit, * withdrawal, and interest.
 */
public class Account {
    ...
    // interest rate of 4.5% private long acctNumber; private float balance; public final String name;

    /**
     * Sets up the account by defining its owner, account number,and initial balance.
     *
     * @param owner   name of account holder * @param account the account number, an identifier for the account
     ...
     * @return true if amount is non-negative, false if amount is negative; indicates balance was not changed.
     */
    public boolean deposit(float amount) {
        boolean result = true; // is amount invalid?
        if (amount < 0) {
            result = false;
        } else {
    ...
    public boolean withdraw(float amount, float fee) { // validate parameters
    ...
    } /* Accessor to the current balance of the account. * @return the current balance of the account. */

    public float getBalance() {
        return balance;
    } /*Accessor to the account number. * @return the account number. */

    public long getAccountNumber() {
        return acctNumber;
    } /*Returns a one-line description of the account as a string. * @return formatted account information */
